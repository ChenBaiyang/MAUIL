...
class CCA(_CCABase):
    ...
    Returns:
        ...
        ev (list): explained variance for each canonical dimension
    """
    def __init__(self, reg=0., numCC=10, kernelcca=False, ktype=None,
...
def kcca(data, reg=0., numCC=None, kernelcca=True, ktype='linear',
    ...
    #crosscovs = [np.dot(ki, kj.T)/len(kj) for ki in kernel for kj in kernel]  ###<--adding 1/n to make the parameter reg resonable.
